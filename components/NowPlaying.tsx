...
function AnimatedBars() {
  ...
  return (
    <div className="flex items-end space-x-1 overflow-hidden">
      <span id="bar1" className="w-2 h-8 bg-gray-300 dark:bg-gray-500 rounded-sm opacity-75" />
      <span id="bar2" className="w-2 h-6 bg-gray-300 dark:bg-gray-500 rounded-sm" />
      <span id="bar3" className="w-2 h-10 bg-gray-300 dark:bg-gray-500 rounded-sm opacity-80" />
    </div>
  );
}
...
export default function NowPlaying() {
  ...
  if (error) {
    return <div className="p-4 bg-red-100 text-red-800 border border-red-300 rounded-md">Error: {error.message}</div>;
  }
