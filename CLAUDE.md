...
- **Make a collection searchable** — global search unions `search_vector` tsvector columns (see `db/migrations/0011_search_vectors.sql`). To add a collection: write a migration adding a `GENERATED ALWAYS AS (...) STORED` tsvector (weight `A` title, `B` facets, `C` body) plus a GIN index, then add a `Source` entry in `lib/search.ts`. Two constraints bite: the expression must be IMMUTABLE (no enum columns; use the `text_array_to_string` wrapper for `text[]`), and the branch's `where` must repeat the collection's public filters — `deleted_at is null`, plus `draft = false` where the table has it — or search will leak trashed and unpublished rows. Never declare the column in `db/schema.ts`; see the note there.
- **Tune related posts** — `getRelatedPosts` in `lib/search.ts` scores candidates as `shared topical tags × 0.35 + ts_rank_cd`, then applies two floors (`RELATIVE_FLOOR`, `MIN_SCORE`). Tags are compared case- and punctuation-insensitively, and `NON_TOPICAL_TAGS` in `lib/search-query.ts` excludes language tags (`English`, `Türkçe`) — they describe a post's form, not its subject, so counting them relates any two English posts. Multi-word tags become adjacency phrases (`self <-> hosting`), never OR-ed halves, or the generic half matches unrelated tags. `MIN_SCORE` is calibrated against the current archive; re-measure if relations start looking wrong.
- **Missing content must rewrite, not redirect** — `writing/[slug]` and `projects/[slug]` answer a bad slug with `Astro.rewrite(`/404?from=<path>`)`. A redirect would answer 302-then-200 (wrong for crawlers) and lose the URL the visitor typed. `Astro.rewrite` does NOT carry the original Request through, which is why the path is passed explicitly as `?from`; `404.astro` sets `Astro.response.status = 404` itself and feeds the path to `search(..., { match: "any" })` for its did-you-mean list. Route words are stripped from that query first — matching them adds noise and prevents the trigram fallback from firing on the misspelled part.
