...
import { getCollection, type CollectionSpec } from "@/lib/backup-collections";
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (dryRun) {
      return json({ ok: true, dryRun: true, mode, summary });
    }
    ...
    return json({ ok: true, dryRun: false, mode, summary });
  } catch (e) {
